...
- **Engraved glyph:** gold line art from `components/Emblems.tsx` in one 1.3-unit stroke on a 64-unit grid (scales, key, compass, crown, velvet rope, queen, hourglass, laurel, letter) plus three ornaments (flourish rule, radiance sun, lozenge crest with the ME monogram). Every stroke carries `pathLength=1`; inside a revealed block it draws on over 1.9s after a 0.3s delay and erases on exit. Placed top-right of each mechanism cell and offer panel, under the FAQ heading and above the application title. The radiance ornament is drawn but not placed: a backdrop version behind the offers heading was removed because it touched the panels.
